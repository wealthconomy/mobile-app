...
export default function CreateFamScreen() {
  ...
  const handlePinComplete = async (enteredPin: string) => {
    ...
    try {
      console.log("👨‍👩‍👧 [WealthFam Create] Verifying PIN...");
      await verifyPin({ pin: enteredPin }).unwrap();
      console.log("✅ [WealthFam Create] PIN verified successfully");
      ...
      const portfolioName = familyCategory
        ? `${familyCategory} - ${membersName.trim()}`
        : membersName.trim() || "Family Wealth Fund";

      const payload: CreatePortfolioRequest = {
        name: portfolioName,
        amount: Math.max(initialDepositKobo, 10000), // Min ₦100 upfront initial contribution
        targetAmount: targetAmountKobo, // Total target goal amount (e.g. ₦1,000,000)
      ...
      console.log("👨‍👩‍👧 [WealthFam Create Request] POST /api/v1/portfolios/wealthfam with payload:\n", JSON.stringify(payload, null, 2));
      ...
      console.log("✅ [WealthFam Create Success] Received response:\n", JSON.stringify(res, null, 2));
      setStep("success");
    } catch (err: any) {
      console.error("❌ [WealthFam Create Error] Failed to create portfolio:\n", JSON.stringify(err, null, 2));
      const message =
        err?.data?.message ||
        err?.message ||
        "Failed to create WealthFam portfolio. Please check your details and try again.";
    ...
    <ScrollView showsVerticalScrollIndicator={false} className="flex-1 px-5">
      ...
      {showInfoCard && (
        ...
        >
          ...
          >
            <Ionicons name="close" size={18} color="#560FF1" />
          </TouchableOpacity>
          <Text
            style={{
              color: "#560FF1",
          ...
          {[
            ...
            >
              ...
              >
                <Text style={{ fontWeight: "700", color: "#560FF1" }}>{item.bold}</Text>
                {item.text}
              </Text>
            </View>
          ))}
      ...
      <View style={{ marginBottom: 20 }}>
        <Text style={styles.inputLabel}>Family Category</Text>
        <TouchableOpacity
        ...
        >
          <Text
            style={{
              color: familyCategory ? "#1A1A1A" : "#9CA3AF",
              fontSize: 15,
              fontWeight: familyCategory ? "600" : "400",
            }}
          >
            {familyCategory || "Select Category (Kids, Spouse, etc.)"}
          </Text>
          <Ionicons
            ...
            {FAMILY_CATEGORIES.map((cat) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  familyCategory === cat && { backgroundColor: TEAL_LIGHT },
                ]}
              >
                <Text
                  style={{
                    color: familyCategory === cat ? TEAL : "#1A1A1A",
                    fontWeight: familyCategory === cat ? "700" : "500",
                  }}
                >
                  {cat}
                </Text>
      ...
      <View style={{ marginBottom: 20 }}>
        ...
        <TextInput
          placeholder="e.g. Sharon, Junior, Dad"
      ...
      >
        ...
        >
          <View>
            ...
            <Text style={styles.previewValue}>{familyCategory}</Text>
          </View>
          <View style={{ alignItems: "flex-end" }}>
            <Text style={styles.previewLabel}>Member</Text>
            <Text style={styles.previewValue}>{membersName}</Text>
          </View>
        </View>
  ...
  const renderPin = () => (
    ...
    >
      ...
      >
        Enter your 4-digit PIN to securely activate your WealthFam portfolio.
      </Text>
  ...
  const renderSuccess = () => (
    ...
    >
      ...
      >
        ...
        You've started a dedicated family pot for{" "}
        <Text style={{ fontWeight: "700", color: "#1A1A1A" }}>
          {familyCategory} ({membersName})
        </Text>{" "}
        with target{" "}
      ...
      <ThemedButton
        title="View WealthFam Portfolios"
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "white" }} edges={["top"]}>
      ...
      <Header
        ...
        onBack={() => {
          ...
          else if (step === "success") router.replace("/(tabs)/portfolios/wealth-fam");
          else router.back();
        }}
      />
